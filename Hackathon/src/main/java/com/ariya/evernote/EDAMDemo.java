package com.alphasense.evernote;/*
...
public class EDAMDemo {
  ...
  private static final String AUTH_TOKEN = "EnterYourToken"; // TODO: Enter your valid dev token
  ...
  public static void main(String args[]) throws Exception {
    ...
    if(args == null || args.length == 0){
      path = "/fse/temp/ariya/evernote"; // Fixme: Enter valid path for htmls to be stored/
    }else{
      path = args[0];
    }
    if (token == null) {
    ...
    EDAMDemo demo = new EDAMDemo(token, path);
    try {
      demo.listNotes();
//      demo.createNote();
//      demo.searchNotes();
//      demo.updateNoteTag();
   ...
   */
  public EDAMDemo(String token, String path) throws Exception {
    ...
    this.path = path +"/"+userStore.getUser().getUsername();
  ...
  private void listNotes() throws Exception {
    ...
    for (Notebook notebook : notebooks) {
      ...
      for (Note note : notes) {
        ...
        FileOutputStream fos = new FileOutputStream(path+"/"+note.getTitle().replace(" ","-")+".html");
        enmlProcessor.noteToInlineHTML(fullNote, fos);
      }
    }
  ...
  private void createNote() throws Exception {
    ...
    Note note = new Note();
    note.setTitle("Test note with AS LOGO for shree Demo.java");

    String fileName = "/Users/ariyabala/Documents/Evernote/mypics/ASlogo.png"; // TODO: Update the path
